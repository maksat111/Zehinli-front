...
const Prizes = () => {
    return (
        <div id='prizes' className='flex flex-col items-center h-190vh lg:grid lg:grid-cols-2 lg:h-90vh'>
            <motion.div initial={{ x: -200, y: 200, scale: 0.3 }} whileInView={{ x: 0, y: 0, scale: 1 }} transition={{ duration: 0.8 }} className='flex h-80vh w-80vw justify-center items-center lg:w-[47vw]'>
                <img className='relative inline-block w-full z-1 h-[30rem] min-w-[18rem] right-[-3rem] lg:h-65vh' src={phoneZehin} alt='Phone' />
                <img className='relative inline-block w-full z-2 h-[26rem] min-w-[19rem] right-[8rem] lg:h-[60vh] lg:min-w-[20rem] lg:right-[9.5rem] lg:bottom-[1rem]' src={prizes} alt=' prizes' />
            </motion.div>
            <div className='flex flex-col justify-center mx-auto px-auto items-start w-80vw lg:w-[47vw] h-80vh'>
                ...
                <motion.h2 initial={{ x: 200, y: 200, scale: 0.3 }} whileInView={{ x: 0, y: 0, scale: 1 }} transition={{ duration: 0.8 }} className='font-extrabold font-lato text-5xl text-new-color py-5'>Bäsleşige gatnaşyň we baýraklar gazanyň</motion.h2>
